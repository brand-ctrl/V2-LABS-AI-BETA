...
st.set_page_config(page_title="V2 LABS AI", page_icon="🧪", layout="wide")

st.markdown("""
<style>
:root { --grad-start:#15aaff; --grad-end:#007bff; }
html, body, [class^="css"]  { background: linear-gradient(135deg, rgba(21,170,255,0.06), rgba(0,123,255,0.06)); }
...
if route == "home":
    ...
    st.caption("Escolha o que deseja fazer")
    c1, c2 = st.columns(2)
    with c1:
    ...
    with c2:
        st.markdown('<div class="v2-card">'
                    ...
                    '<p>Baixe imagens a partir de um CSV com URLs.</p>'
                    '</div>', unsafe_allow_html=True)
    ...
    st.subheader("Sobre")
    st.write("Suite de ferramentas de imagem **V2 LABS AI**.")
